...
import {
  DisplayStateFieldsType, DisplayStateGaugeFieldsObject, DisplayStateNeedleFieldsObject
} from "../../config/display/state/display.state";
import {GaugeConfig, NeedleConfig} from "../../config/display/models/configs";
import {
  selectGaugeConfigs, selectHighlight, selectNeedleConfigs
...
export class GaugesComponent implements AfterViewInit, OnDestroy {
  ...
  constructor(private readonly store: Store) {
    this.subscription.add(this.store.select(selectAllImages).subscribe((images) => {
      this.images = images;
      this.redrawAll();
    }));
    this.subscription.add(this.store.select(selectNeedleConfigs).subscribe((configs) => {
    ...
    this.subscription.add(this.store.select(selectHighlight).subscribe((highlight) => {
      this.highlight = highlight;
      this.redrawAll();
    }));
    this.subscription.add(this.store.select(selectNeedleAngles).subscribe((angles) => {
  ...
  drawNeedles() {
    ...
    if (this.images.needleImage1) {
      ...
      this.compositor.drawImageRotatedAround(this.images.needleImage1, config.positionX!,
        config.positionY!, config.positionX! + config.centerX!, config.positionY! + config.centerY!,
        this.needleAngles![0]);
    }
    if (this.images.needleImage2) {
      const config = this.needleConfigs.needle2;
      this.compositor.drawImageRotatedAround(this.images.needleImage2, config.positionX!,
        config.positionY!, config.positionX! + config.centerX!, config.positionY! + config.centerY!,
        this.needleAngles![1]);
    }
    if (this.images.needleImage3) {
      const config = this.needleConfigs.needle3;
      this.compositor.drawImageRotatedAround(this.images.needleImage3, config.positionX!,
        config.positionY!, config.positionX! + config.centerX!, config.positionY! + config.centerY!,
        this.needleAngles![2]);
    }
  }
